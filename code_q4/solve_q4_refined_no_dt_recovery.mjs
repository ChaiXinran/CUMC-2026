...
function simulateQuestion4(environment, radiusModel, options = {}) {
  ...
  // 初始预热阶段允许从 initialDt 逐步恢复到 requestedDt；
  // 一旦出现拒步，则锁定降低后的 dt，不再自动恢复。
  let lockReducedDt = false;
  let outputIndex = 0;
  ...
  while (t < tEnd - 1e-10) {
    ...
    try {
      ...
      // 初始预热阶段允许逐步恢复到 requestedDt。
      // 一旦曾经发生拒步，则保持降低后的 dt，不再自动恢复。
      if (!lockReducedDt) {
